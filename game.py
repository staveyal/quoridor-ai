...
class AlphaBetaPlayer(Player):
    ...
    def __recursive_minimax(self, game_state, depth, is_max, best_other):
        ...
        for next_action in game_state.get_legal_moves():
            next_state = game_state.generate_successor(next_action)
            next_value, _ = self.__recursive_minimax(next_state, depth - 1 if not is_max else depth, not is_max, value)
            if is_max:
                if value < next_value:
                    value = next_value
                    action = next_action
...
if __name__ == '__main__':
    quoridor = Quoridor(
        lambda id, pos, goal: create_player(id, pos, goal),
